...
function extractArray(payload, name) {
  if (payload?.status === 'error') throw new Error(`${name}: ${payload.message || 'Backend returned error status'}`);
  if (payload?.status !== 'success') throw new Error(`${name}: unexpected status "${payload.status}"`);
