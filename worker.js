export default {
  async fetch(request, env, ctx) {
    ...
    if (!apiKey) {
      return new Response("VGUMAP_3D_VIEW not configured", {
        status: 500,
        headers: { "Access-Control-Allow-Origin": "*" }
      });
    }
    ...
    // Stable cache key without query string
    const cacheKey = new Request(
      `https://vgumap-3d-cache/${fileId}`,
      { method: "GET" }
    );
    ...
    if (!driveResponse.ok) {
      return new Response(`Google Drive error: ${driveResponse.status}`, {
        status: driveResponse.status,
        headers: { "Access-Control-Allow-Origin": "*" }
      });
    }

    // Read body once only (avoids "body already used" error)
